...
def provider_scope(provider: str, api_key: str | None = None) -> str:
    if not api_key:
        return provider
    # The secret itself never appears in Redis keys, logs, or persisted work items.
    return f"{provider}:key:{sha256(api_key.encode()).hexdigest()[:16]}"
...
async def acquire(redis: Redis, settings: Settings, provider: str, stage: str, now_ms: int,
                  estimated_tokens: int = 0, scope: str | None = None) -> bool:
    limit = provider_limit(settings, provider, stage)
    scope = scope or provider
    total_key = f"stem:limit:{scope}:total:inflight"
    lane_key = f"stem:limit:{scope}:lane:{limit.lane}:inflight"
    rpm_key = f"stem:limit:{scope}:rpm"
    tpm_key = f"stem:limit:{scope}:tpm"
...
async def release(redis: Redis, settings: Settings, provider: str, stage: str,
                  scope: str | None = None) -> None:
    limit = provider_limit(settings, provider, stage)
    scope = scope or provider
    await redis.eval(RELEASE_LUA, 2, f"stem:limit:{scope}:total:inflight", f"stem:limit:{scope}:lane:{limit.lane}:inflight")
