...
@pytest.mark.asyncio
async def test_doubao_thinking_is_enabled_for_solve_and_disabled_for_equivalence(monkeypatch) -> None:
    request_bodies = []
    ...
    assert "只输出最终答案本身" not in request_bodies[0]["messages"][0]["content"]
...
async def test_difficulty_solve_requests_only_the_final_answer(monkeypatch) -> None:
    ...
    assert "只输出最终答案本身" in request_bodies[0]["messages"][0]["content"]
