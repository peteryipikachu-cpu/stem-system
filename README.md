...
一个面向 STEM 题库的题目导入、异步 AI 审核和结果追溯系统（产品界面名「AI 质检系统」）。前端用于题目管理、导入和人工复核；API 负责鉴权、数据与任务编排；Worker 从队列中消费审核任务并调用模型。
...
顶部导航包含项目管理、用户管理、模型管理、队列监控与健康巡检五个一级入口。

**项目管理**：项目卡片展示题目目标与完成进度、累计调用成本与请求数、成员构成（项目管理员 / 普通用户），支持编辑、学科分布、暂停、停止、删除，以及项目级调用成本分析。

![项目管理](docs/images/project-list.png)

**题目列表**：状态 / 是否合格 / 分级状态 / 难度等级 / 提交人 / 日期等多维筛选；全部题目、待检测、疑似合成、合格四张指标卡；支持批量质检、Excel 导入导出与版本追溯。

![题目列表](docs/images/question-list.png)

**题目详情**：左侧查看题干、答案与解题思路（KaTeX 渲染）；右侧实时跟踪难度分级评测各层进度与多次独立求解结果，保留 AI 合成题检测与相似题检测的判断依据。

![题目详情与质检结果](docs/images/question-detail.png)

**用户管理**：管理员 / 项目管理员 / 普通用户三级账号；支持 Excel 批量上传（用户名 + 密码）、下发新账号、可访问项目分配、题目上传上限与日 / 月 / 月预算生效配额，并展示每账号累计请求次数与 RMB 成本。

![用户管理](docs/images/user-management.png)

**模型管理 · 调用治理**：配置范围支持全局与单项目；功能开关（相似度检测总开关）；全局网关与豆包 / Gemini / DeepSeek / Qwen / Kimi / Claude 各厂商的并发、RPM、TPM 双层容量限额，多用户公平调度与重试参数、全局账号配额默认值、模型分档单价与计费币种，调整即时生效并在任务创建时快照固化。

![模型管理与调用治理](docs/images/model-management.png)

![厂商容量与调度参数](docs/images/model-governance.png)

**模型管理 · 分层策略**：答案比对模型独立配置；L1 过易筛选 / L2 难度复核 / L3 终极定级三层各带启用开关，可分别配置作答模型、运行次数（Pass@K）、答对门槛与单次超时，满足门槛逐层晋级、不满足退回定级。

![分层策略配置](docs/images/difficulty-policy.png)

**队列监控**：Worker 在线状态，排队中 / 执行中 / 等待依赖 / 已暂停等队列统计，共享网关与各厂商容量占用，以及活跃任务的进度与卡住诊断。

![队列监控](docs/images/queue-monitor.png)

**健康巡检**：相似校验卡住、队列缺失 / 残留、阻塞停滞、排队过久、Worker 心跳、未定稿任务与未结算台账等实时检测结果；Worker 每 5 秒自动修复，本页面供管理员人工兜底。

![健康巡检](docs/images/health-patrol.png)
