...
- 题目上传规范拦截（后端 `services.upload_rule_violations`）：`POST /api/questions` 精确去重后逐条校验规则可判定项，不合规跳过并经响应 `blockedInvalid`（标题+原因）回报，其余照常导入；`PUT /api/questions/{id}` 保存新版本前同口径校验（违规 422）防绕过。规则：题目类型非空须含"解答题/计算题"；题干含证明/求证/判断题/对或错/选择题关键词或 ≥3 个 A-D 选项行拦截；≥2 个不同小问编号（(1)/(2)、①②、(I)）违反单题单问；答案含 ≈/约等于/\approx 视为近似小数；答案剥离 LaTeX 命令与定界符后取值不得为 0/1/-1（兼容 `x=0` 前缀）、长度 ≤40 字符；答案含推导连接词（所以/因此/故/得到/设/由于/代入/求导/积分/解得/则）或 ≥3 行视为解题思路混入答案列，同样拦截；题干为是否类封闭问法（含是否/能否/成立吗等二值问句标记且以问号收尾）而参考答案为条件式（当且仅当/不一定/仅当/取决于/只有当/而定）视为问答形式错配，同样拦截（二值问法会逼模型输出"是/否"、与条件式答案比对判错，应改为求条件或论述式问法）。"非知识问答/需复杂推理/答案唯一/未公开/人工原创"等语义要求不做上传拦截，由上传后 L0 合成题检测与难度分级评测异步覆盖，勿在上传链路引入同步 AI 判定。
