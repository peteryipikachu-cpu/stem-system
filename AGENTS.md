...
- 历史质检记录可追溯：`GET /api/questions/{id}/check-runs/history` 返回题目 全部历史 run（含已取消/失败）及其台账调用次数/成本汇总，用量明细复用 `/api/check-runs/{id}/usage`（逐条明细附工作项 `result`，用量弹窗展开行可查看模型产出：答案/思考、等价判定、合成检测结论）；可见性沿用题目所有者范围，前端详情页"历史质检任务"卡片仅 admin/project_admin 展示。重新质检只切换 `difficulty_run_id` 引用，不得删除或覆盖历 史 run 与结果记录。
