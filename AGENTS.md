...
- 题目上传规范拦截（后端 `services.upload_rule_violations`）：`POST /api/questions` 精确去重后逐条校验规则可判定项，不合规跳过并经响应 `blockedInvalid`（标题+原因）回报，其余照常导入；`PUT /api/questions/{id}` 保存新版本前同口径校验（违规 422）防绕过。规则：题目类型非空须含"解答题/计算题"；题干含证明/求证/判断题/对或错/选择题关键词或 ≥3 个 A-D 选项行拦截；≥2 个不同小问编号（(1)/(2)、①②、(I)）违反单题单问；答案含 ≈/约等于/\approx 视为近似小数；答案剥离 LaTeX 命令与定界符后取值不得为 0/1/-1（兼容 `x=0` 前缀）、长度 ≤40 字符；答案含推导连接词（所以/因此/故/得到/设/由于/代入/求导/积分/解得/则）或 ≥3 行视为解题思路混入答案列，同样拦截；题干为是否类封闭问法（含是否/能否/成立吗等二值问句标记且以问号收尾）即判断题式问法，无论答案形式同样拦截（二值问法会逼模型输出"是/否"、与条件式答案比对判错，应改为求条件或论述式问法）。"非知识问答/需复杂推理/答案唯一/未公开/人工原创"等语义要求不做上传拦截，由上传后 L0 合成题检测与难度分级评测异步覆盖，勿在上传链路引入同步 AI 判定。
...
- L0 包含本地 Markdown/LaTeX 校验（不调用模型）与 AI 合成题检测（项目配置的 synthesis 模型优先，未配置时兜底 `deepseek-v4-flash`；兜底常量由三端 `audit_models.py` 的 `SYNTHESIS_AUDIT_MODEL_ID` 与后端 `DEFAULT_GLOBAL_AUDIT_MODEL_IDS["synthesis"]` 共同决定，修改时须三处同步）；两者均为 `difficulty_assessment` 的 L0 工作项。合成题检测 Prompt 兼任“是否构成正常题目”判定：孤立数字/片段等非题目内容输出 `is_valid_question=false`，L0 门控据此将评测定为 `invalid_question` 阻断定级（前端展示“非正常题目”）；旧检测结果无该字段按有效兼容，检测调用失败不阻塞分级。L0 门控同时兜底上传规范可判定项：是否类封闭问法（口径同 `upload_rule_violations`，Worker `binary_question_rule_reason`）的存量题（规则上线前入库）在晋级 L1 前同样定为 `invalid_question`，failure 文案标注“不符合上传规范”，不消耗 L1+ 作答调用（题目 279 案例）。
