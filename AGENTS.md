...
- GitLab CI 的 npm 依赖安装（前端 `build:static`）统一配置淘宝镜像 `NPM_CONFIG_REGISTRY=https://registry.npmmirror.com`，并满足两条缓存纪律：① 依赖安装用 `npm install`（增量复用）而非 `npm ci`——`npm ci` 会先删除 `node_modules` 再全量重装，使 GitLab 恢复的 node_modules 缓存完全失效；② 除 node_modules 缓存外另配 Next.js 增量构建缓存（`app/.next/cache/`，按 `CI_COMMIT_REF_SLUG` 键），加速 `next build`。调整前端 CI 时保持同配置。
...
- 项目管理员账号管理（`/api/project-users`）：项目管理员仅能查看、下发、编辑其所在项目内的普通用户；下发固定 `role="user"`，项目分配仅限请求者所在项目（越权 403）；编辑时保留请求者范围外的项目归属，非普通用户或不在其范围内的账号不可见（404）。角色升降仍为管理员专属；管理员全量账号管理继续走 `/api/users`。项目逻辑删除：管理员可删任意项目，项目管理员可删自己所在的项目（`DELETE /api/projects/{id}`，后端按 `project_scope` 校验）；删除后账号列表序列化的 `projectIds`/`projects` 同步排除已删除项目（2026-08-11 修复，此前编辑用户弹窗会把已删除项目显示为裸 id），`project_user_view` 经 `manager_project_ids` 天然过滤。用量分析（`GET /api/admin/project-costs`）：管理员可见全部项目与全部用户的调用成本汇总；项目管理员同样可访问，但按 `project_scope` 仅返回其所属项目的汇总，用户汇总也仅统计这些项目范围内的请求（无相关请求的用户不展示）；普通用户仍返回 403。前端“我的项目”页的“调用成本”按钮与项目卡片累计成本对 admin 和 project_admin 均展示。
- 个人题目上传上限：`User.question_upload_limit`（null=不限制），可在 `/api/users` 与 `/api/project-users` 的下发/编辑接口设置；`POST /api/questions` 按提交者名下题目总数门控，达上限或单次提交超出剩余配额时返回 409 并提示剩余可上传数量。账号列表接口附带 `questionCount`（已上传数）供前端展示消耗。项目题目目标门控（上传/质检/重新开始/项目更新时的达标判定、自动暂停与剩余名额）统一按合格题数统计（`project_target_reached` 复用 `count_qualified_questions`，口径同进度展示），不再按题目总数；上传本身不触发自动暂停，合格数达标由 Worker 在分级评测定级落库时同事务触发（`pause_project_if_target_reached`，口径镜像后端：项目级 `qualifiedMinLevel` 覆盖优先、回退全局策略）。
...
- 批量上传用户：`POST /api/users/batch`（管理员）与 `POST /api/project-users/batch`（项目管理员，项目仅限自己所在项目），Excel 固定表头仅“用户名、密码”（2026-08-11 起移除手机号列），密码由文件逐行填写并须满足复杂度（≥8 位且含大小写字母、数字、特殊字符；前端解析时拦截“用户名或密码任一为空/不合规”的行并禁用提交，后端 `batch_create_users_core` 逐行复核、不合规整批 400 带行号），日上限/月上限/月预算作为弹窗内统一输入框对本批次所有账号生效（留空继承全局默认值），题目上传上限同样统一输入（留空不限制），账号固定 `role="user"`；用户名已存在或文件内重复的行跳过并在 `skipped` 中逐条说明，不阻断其余创建。手机号不再进入批量文件，`User.phone`（可空）仍可通过单发（`/api/users`、`/api/project-users`）与编辑接口的 `phone` 字段填写与清空（传 null 清除）。
...
- 题目上传规范拦截（后端 `services.upload_rule_violations`）：`POST /api/questions` 精确去重后逐条校验规则可判定项，不合规跳过并经响应 `blockedInvalid`（标题+原因）回报，其余照常导入；`PUT /api/questions/{id}` 保存新版本前同口径校验（违规 422）防绕过。规则：题目类型非空须含"解答题/计算题"；题干含证明/求证/判断题/对或错/选择题关键词或 ≥3 个 A-D 选项行拦截；≥2 个不同小问编号（(1)/(2)、①②、(I)）违反单题单问；答案含 ≈/约等于/\approx 视为近似小数；答案剥离 LaTeX 命令与定界符后取值不得为 0/1/-1（兼容 `x=0` 前缀）、长度 ≤40 字符；答案含推导连接词（所以/因此/故/得到/设/由于/代入/求导/积分/解得/则）或 ≥3 行视为解题思路混入答案列，同样拦截；题干为是否类封闭问法（含是否/能否/成立吗等二值问句标记且以问号收尾）即判断题式问法，无论答案形式同样拦截（二值问法会逼模型输出"是/否"、与条件式答案比对判错，应改为求条件或论述式问法）；参考答案含未被 $ 包裹的裸公式同样拦截（剔除 $…$、\(…\)、\[…\] 定界段后仍残留 LaTeX 命令或 ^/_ 即裸公式，前端无法渲染、模型比对易误判；纯数字/`x=2` 类短答案不触发）。"非知识问答/需复杂推理/答案唯一/未公开/人工原创"等语义要求不做上传拦截，由上传后 L0 合成题检测与难度分级评测异步覆盖，勿在上传链路引入同步 AI 判定。
- 历史质检记录可追溯：`GET /api/questions/{id}/check-runs/history` 返回题目 全部历史 run（含已取消/失败）及其台账调用次数/成本汇总，用量明细复用 `/api/check-runs/{id}/usage`（逐条明细附工作项 `result`，用量弹窗展开行可查看模型产出：答案/思考、等价判定、合成检测结论；失败/中断行默认展开，长文本经 Drawer 查看全文）；可见性沿用题目所有者范围，前端详情页"历史质检任务"卡片仅 admin/project_admin 展示。重新质检只切换 `difficulty_run_id` 引用，不得删除或覆盖历 史 run 与结果记录。
- 模型调用、重试、并发和限流集中在 `services.py` 与 `config.py`。新增审核类型应经过队列、依赖激活、结果落库、完成状态和事件发送的完整链路。
- 相似题检测分两层：上传时同步拦截 + 上传后异步模型判定。上传拦截（`POST /api/questions` 的 `filter_similar_uploads`）只做本地 n-gram 词汇召回，阈值 `SIMILARITY_UPLOAD_BLOCK_THRESHOLD=0.5`（高于异步召回线 0.18，仅拦近乎相同者，同批次已接受项也在内存互查），被拦条目不导入并随响应 `skippedSimilar` 返回（`skipped` 仍仅统计精确重复；上传去重是精确文本匹配，仅空白差异的同题靠拦截/相似题检测兜底）；改写幅度较大的同题不拦，由上传后异步模型判定标记。召回依赖 `question_ngrams` n-gram 索引，功能上线前入库的历史题目曾无索引导致召回为空直接判 `clear` 的盲区（题目 127 与题目 8 同题未发现）；用 `stem-system-backend/app/scripts/backfill_question_ngrams.py` 幂等补齐索引，`--rescan <题目id>` 可清理旧 similarity 任务（idempotency_key 冲突，关联记录级联删除）后重新召回判定；重扫时 `CheckRun.requested_by_user_id` 须传真实用户或 None，外键不接受不存在的 id。`similarity_status` 四种终态语义：`checking`（进行中）→ 成功写 `clear`/`suspected`，工作项进 manual_review/dead 写 `failed`（Worker `complete_run_if_ready` 必须覆盖失败路径，否则题目永久卡 checking 无法质检）；三种非 clear 态都锁定质检，管理员可经 `PUT /api/questions/{id}/similarity-resolution` 解除，检测失败另可复位工作项（清 error/completed_at/租约并同步重入 Redis 公平队列四个索引）重跑。相似题检测按产品口径（2026-08-12）不受日上限/月上限/月预算与质检次数限制、也不消耗这些限额的次数：后端 `create_similarity_run` 不做配额预检（旧预检超限不创建直接标 clear 同属限制，已移除），Worker 派发门控统一走 `dispatch_quota_violation` 对 `check_type == "similarity"` 直接豁免（日/月计数统计本就排除 similarity run）；两端必须同口径，否则"创建后超限/创建后调低上限"场景 similarity 工作项每 60 秒重试永不通过、题目永久卡 checking（CosmosMind 月上限 16 已用 29 案例）。
- 已定级且合格的题目禁止重新质检（后端 `require_recheck_allowed`，判定复用 `question_is_qualified`：当前版本基础检测通过且分级完成、难度 ≥ 合格最低门槛）：合格最低门槛 `qualifiedMinLevel` 可配置——全局难度策略存默认值、项目可在项目级难度策略覆盖（L1/L2/L3，L0 不可选，默认与存量回退 L2），判定口径每次实时读库、保存即生效；单题质检（`POST /api/questions/{id}/check`）、难度分级（`/difficulty-assessments`，force 不绕过）与队列重试（`/api/admin/queue/check-runs/{id}/retry`）命中直接 409；批量质检（`POST /api/check-batches`）自动剔除合格题目继续执行剩余部分，被剔除题目随响应 `skippedQuestionIds` 返回，全部命中则 409。仅 `role == "admin"` 豁免，项目管理员与普通用户一律拦截。改版本天然放行：保存新版本使 `current_version += 1`，新版本无检测结果即不再合格，无需额外重置逻辑。上传/保存新版本自动触发的相似题检测不经过这些入口，不受影响。前端须同步门控：详情页分级卡“重新检测”按钮禁用并 tooltip 说明，批量确认框提示将跳过的合格题数量、提交后按 `skippedQuestionIds` 提示并修正 run 映射/进度总数。
...
- Worker 完成与失败事务都必须校验工作项终态：重启/恢复/租约误回收可能造成同一工作项被并行重复执行，迟到的完成不得覆盖结果或再次触发比对/收尾级联（完成前检查 `status == running` 且 `completed_at` 为空）；迟到的失败同理不得把已 completed 的工作项翻回 failed 或用空答案覆盖真实结果——失败落库前做同样的终态检查，迟到失败仅照常结算台账（上游调用真实发生）后直接返回，不翻状态、不写 error/result、不重试、不触发级联（题目 276 案例，回归用例 Worker `tests/test_stale_identity_map.py::test_late_failure_does_not_overwrite_completed_work`）。同理 `update_assessment_progress` 等进度回执不得把已由比对定稿（pass/fail）的层级倒回 `equivalent=None` 的中间态，否则前端会永久显示“待比对”。
...
- 中断台账按流式进度记部分消耗（`mark_ledger_interrupted`）：Worker 关闭取消用内存 `stream_counters`，重启/崩溃的孤儿回收读工作项 `payload["stream"]`（流式观察每 2 秒经 `refresh_running_work_lease` 持久化 contentChars/reasoningChars）；输出估算 = (contentChars+reasoningChars+3)//4，与正常本地估算同口径（思考文本计入输出计费、思考 Token 单列不重复计费），消息附"已按已接收流式进度估算部分消耗"。非流式调用无 chunk 进度，仍只记输入估算。回归：Worker `tests/test_orphaned_ledgers.py`。
...
- 重新分级/重新检测增量复用：创建新任务或触发重试时只把失败、超时或未完成的作答工作项重新入队；历史已完成的作答（含空答案）与 L0 检测工作项直接继承（置 `completed`、拷贝结果与耗时、payload 标记 `inheritedFrom`），跳过 Redis 入队以节省上游配额；例外：`assessment_format` 失败（含错误条目）的历史结果不继承——L0 规则校验结果随规则版本变化且重跑零成本，必须用当前规则重新检测。空答案作答同样直接继承不重跑（2026-08-12 口径调整：空答案重跑大概率仍是空答案，重跑只浪费上游配额），继承后由下条“分级评测空答案作答”口径按答错计 0 次。复用源从最近一次历史 `CheckRun` 向前追溯（限同题同版本、最多回看 5 次），跨 run 累计、同键取最新 run 的结果。匹配键为（层级, 模型, 该模型在本层的第几次作答）：payload 有 `modelAttempt` 直接用，旧数据回退为同层同模型内按 attempt 升序的序号，策略里模型增减/顺序变化后仍能对齐；序号必须在包含失败/取消项的全量集合内计算，`fetch_reusable_solves` 返回已带序号的元组，调用方不得重新编号，否则与展开侧 attempt/modelAttempt 错位失配。若所有作答均被继承，比对任务（`equivalence`/`assessment_equivalence`）由后端或 Worker 直接唤醒，不得遗留永久阻塞。`fetch_reusable_solves`/`inherit_successful_solves` 在后端与 Worker 两个仓库各有一份同名实现（后端负责创建任务时初始工作项的继承，Worker 负责分级层展开时作答工作项的继承），复用口径变更必须两端同改并同步各自 `tests/test_inherit_solves.py`，否则会出现“后端已修、Worker 仍按旧口径继承”的错位（2026-08-12 空答案口径两次调整均需两端同改，其中不继承改造曾因此漏了 Worker 侧）。
- APIRoute 模型接入：新增 OpenAI-compatible 网关模型时，须同步更新前端、后端与 Worker 的 `audit_models` 目录；统一使用 `provider="apiroute"`，共享 `APIROUTE_API_KEYS` 和 APIRoute 并发/限流通道。未提供专属规则时，默认 Pass@K 3、难度答对阈值 ≤2，深度思考参数保持关闭。模型 ID 以网关实际注册名为准：kimi 于 2026-08-07 改为带厂商前缀的 `kimi/kimi-k3`（2026-08-12 探针发现旧 ID `kimi-k3` 恢复可用、裸名 `kimi` 仍 503，但渠道与计费口径无法确认，canonical ID 保持 `kimi/kimi-k3` 不变），改名须同步三端目录、思考参数匹配、治理厂商/价目表与 DB `system_settings` 的 `difficulty_policy`/`model_governance`。
...
- kimi 厂商并发于 2026-08-11 由 2 提升至 6（L3 的 kimi/kimi-k3 运行次数 3，2 并发槽长期饱和）：后端与 Worker 代码默认值同步；因库中已存值覆盖代码默认值且上线流水线无人工 SQL 环节，后端 lifespan 启动时另跑幂等修复 `raise_kimi_vendor_concurrency`，把 `model_governance` 已存值低于下限 `KIMI_CONCURRENCY_FLOOR=6` 的统一提升。将来若需下调须同步调整或移除该修复；管理员页仍可随时调高。
...
- 仅 `408/429/500/502/503/504`、连接/超时/协议中断等临时网络错误、PostgreSQL 死锁/序列化失败（SQLSTATE `40P01`/`40001`，Worker `provider_error` 归为可重试的 `db_transient_error`）以及上游“成功”但既无答案内容也无思考内容的空完成（`empty_completion`，模型调用错位，Worker `call_chat` 抛 `EmptyCompletionError`；如 kimi/kimi-k3 曾返回 delta 全空、finish_reason=stop、usage 有 completion_tokens 的流）可重试；最多 5 次重试，退避 `2/4/8/16/32` 秒并加 `0~1` 秒抖动。kimi/kimi-k3 的 `The engine is currently overloaded, please try again later` 为官方 `engine_overloaded_error`（HTTP 429），落在可重试集合内走同一退避；`friendly_http_message` 按 error code 区分两类 429——`insufficient_user_quota`/用户额度不足翻译为充值提示、overloaded 保留原文，不得混淆。退避总窗口约 62 秒，可吸收瞬时过载尖峰；过载持续数分钟超出窗口时尝试耗尽按终态规则处理（分级阶段整层判败带原因、作答转人工复核），配合 kimi 厂商并发下限修复（见上条）缓解长期饱和。`400/401/403/404/405/409/410/413/415/422`、无效模型/参数/响应结构和缺失密钥直接失败；退避期间不占任何执行槽位。数据库瞬态错误不得落入不可重试的 `execution_error`：一次偶发死锁若直接判作答失败，会在比对定稿时触发“该层存在未成功完成的模型作答”整层判败（题目 39 首次评测即因此失败）。空完成同样不得把空文本当正常作答落库——旧链路经“空答案=答错”代入 K 判定，异常调用反而推动层级晋级；耗尽重试后按“该层存在未成功完成的模型作答”整层判败/人工复核，工作项与台账记录 `empty_completion`。
...
- L0 包含本地 Markdown/LaTeX 校验（不调用模型）与 AI 合成题检测（项目配置的 synthesis 模型优先，未配置时兜底 `deepseek-v4-flash`；兜底常量由三端 `audit_models.py` 的 `SYNTHESIS_AUDIT_MODEL_ID` 与后端 `DEFAULT_GLOBAL_AUDIT_MODEL_IDS["synthesis"]` 共同决定，修改时须三处同步）；两者均为 `difficulty_assessment` 的 L0 工作项。合成题检测 Prompt 兼任“是否构成正常题目”判定：孤立数字/片段等非题目内容输出 `is_valid_question=false`，L0 门控据此将评测定为 `invalid_question` 阻断定级（前端展示“非正常题目”）；旧检测结果无该字段按有效兼容，检测调用失败不阻塞分级。L0 门控同时兜底上传规范可判定项：是否类封闭问法（`binary_question_rule_reason`）与参考答案裸公式（`bare_formula_rule_reason`），口径同 `upload_rule_violations`，存量题（规则上线前入库）在晋级 L1 前同样定为 `invalid_question`，failure 文案标注“不符合上传规范”，不消耗 L1+ 作答调用（题目 264/279/285 案例）。
- LaTeX 规则引擎三处放宽（2026-08-11，误伤修正，按"合法 TeX 语法不拦、只拦真缺陷"口径，不得再收紧回误报形态）：① `latex-command-arg` 按 TeX 单记号参数语义校验——任何受检命令的参数可以是平衡 `{…}` 组或单记号（单个字母/数字/反斜杠命令），`\mathbb F_2^4`、`\sqrt x`、`\frac13`、`\frac 12` 均合法不报缺参数；缺参数（裸 `\sqrt`）与缺分母（`\frac{3}`、`\frac 1`）仍拦截。② `latex-unwrapped-math-command` 豁免纯公式行——整行不含中文/全角、含数学命令且无纯字母散文词（如 `U_{yx}=\frac{...}4.`、`\lvert\phi\rangle=\sqrt{...}`）不视为"正文"；中英混排句子里裸写命令仍拦截。③ `latex-subscript-digit` 扫描前先等长剔除反斜杠命令，命令名与数字简写粘连（`\frac13` 的 `c13`）不再误报丢下标；真实 `dx2`/`x1` 仍报。排版风格类建议（`\mathbb{F}` 全写法、`\,` 小间距、`\lvert` 紧凑间距、比特串 `\mathrm` 正体）属美观优化，规则引擎一律不拦。前端 `LatexRenderer` 同步兜底：无 `$` 包裹的纯公式段整体按块级 KaTeX 渲染（失败退回片段兜底），与 Worker 豁免同口径，避免放宽后详情页显示原始 `\frac` 文本。
- `_math_segments` 闭合顺序红线（2026-08-11 修复）：段内扫描必须先查闭合符再走反斜杠转义分支——`\]`/`\)` 本身以反斜杠开头，顺序颠倒会把闭合符当转义对吞掉，`\(...\)` 与 `\[...\]` 段永远无法闭合（公式内容全部落回"正文"，`\sum`/`\lambda` 被误报裸命令、公式内公式不被校验）。`$...$` 闭合符无反斜杠故旧实现侥幸正常。同批修复 `md-table-columns`：统计表格列数前先把数学片段替换为等长空白，狄拉克记号 `|1,1,1,0\rangle` 的竖线不再误判为表格分隔符。
- 所有模型均通过 OpenAI-compatible APIRoute 网关请求；逻辑 Provider 仅用于区分参数和限流策略。
- 解题 `max_tokens` 不再硬编码：Worker 派发每个工作项时实时读取治理配置 `solveMaxTokens`，留空则不传；管理员在“模型管理”页修改保存后对新派发的工作项即时生效，无需重启或改代码。传参字段按模型区分：kimi/kimi-k3 官方已废弃 `max_tokens`，须以 `max_completion_tokens` 传递（官方默认输出 128K、最大可设 1M、上下文窗口 1M）；其余模型仍用 `max_tokens`。
...
| claude-sonnet-5 | `thinking.type=adaptive`；`output_config.effort=max` | `thinking.type=disabled` | `thinking.type=adaptive`；`output_config.effort=high` | 仅解题流式 |

- `glm-5.2` 已移除，不得重新加入默认策略或模型目录，除非完成可用性验证并明确授权。
- Claude 系列思考参数按官方 adaptive thinking 格式：必须显式传 `thinking.type="adaptive"` 否则模型不思考，`effort` 放独立顶层 `output_config`（放进 `thinking` 会 400）；Sonnet 仅支持 low/medium/high，`max`/`xhigh` 为 Opus 专属（Sonnet 传 `max` 官方会报错）。2026-08-07 实测 APIRoute 的 claude-sonnet-5 通道：含官方格式在内六组思考参数均无 `reasoning_content` 返回且耗时仅 3~7 秒，判定网关未透传思考参数（探针 `scripts/probe_claude_thinking.py`，支持 `--extra NAME=JSON` 复测）；网关升级后须复测，若思考生效需同步成本估算的思考 Token 口径。2026-08-12 线上出现 claude-sonnet-5 `empty_completion`（流式 HTTP 200 正常收尾但无任何 content/reasoning 片段、`completion_tokens=0`）：属上游瞬时空响应，Worker 按可重试协议异常退避重试，空文本不会代入 K 判定；当日用探针五组参数复测均正常返回内容，通道与参数无回归，重试耗尽判败的层级直接重跑即可。
