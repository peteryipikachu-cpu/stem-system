...
- 批量上传用户：`POST /api/users/batch`（管理员）与 `POST /api/project-users/batch`（项目管理员，项目仅限自己所在项目），Excel 固定表头仅“用户名、密码”（2026-08-11 起移除手机号列），密码由文件逐行填写并须满足复杂度（≥8 位且含大小写字母、数字、特殊字符；前端解析时拦截“用户名或密码任一为空/不合规”的行并禁用提交，后端 `batch_create_users_core` 逐行复核、不合规整批 400 带行号），日上限/月上限/月预算作为弹窗内统一输入框对本批次所有账号生效（留空继承全局默认值），账号固定 `role="user"`；用户名已存在或文件内重复的行跳过并在 `skipped` 中逐条说明，不阻断其余创建。手机号不再进入批量文件，`User.phone`（可空）仍可通过单发（`/api/users`、`/api/project-users`）与编辑接口的 `phone` 字段填写与清空（传 null 清除）。
