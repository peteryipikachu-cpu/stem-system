...
- 分段计价：治理配置 `models[*].priceTiers` 按 `maxInputTokens` 升序定义多档单价（默认 doubao-seed-2-0-pro-260215 三档：≤32K 3.2/16、≤128K 4.8/24、≤256K 9.6/48 元/M），扁平 `inputPricePerMillion/outputPricePerMillion` 仅作无分档兜底。结算时按真实输入 token 选档（首个 `maxInputTokens ≥ 输入` 的档，超出末档按末档），并把命中档单价回写台账 `input/output_price_per_million` 两列；上游 usage 含 `prompt_tokens_details.cached_tokens` 时命中部分按档内 `cacheHitPricePerMillion` 计价（缺省回退档内输入价），缓存存储价按时长计费、无法归属单次请求，不纳入请求成本。
