...
- LaTeX 规则引擎两处放宽（2026-08-11，误伤修正，不得再收紧回误报形态）：① `latex-command-arg` 允许单记号参数——`\mathbb`/`\hat`/`\vec`/`\sqrt` 等单参数命令后接单个字母、数字或命令（如 `\mathbb F_2^4`、`\sqrt x`）是合法 LaTeX，不报缺花括号；`\frac` 系双参数与 `\text` 仍要求显式花括号（`\frac 12` 仍拦截）。② `latex-unwrapped-math-command` 豁免纯公式行——整行不含中文/全角、含数学命令且无纯字母散文词（如 `U_{yx}=\frac{...}4.`、`\lvert\phi\rangle=\sqrt{...}`）不视为“正文”；中英混排句子里裸写命令仍拦截。前端 `LatexRenderer` 同步兜底：无 `$` 包裹的纯公式段整体按块级 KaTeX 渲染（失败退回片段兜底），与 Worker 豁免同口径，避免放宽后详情页显示原始 `\frac` 文本。
