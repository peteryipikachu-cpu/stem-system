...
- 个人题目上传上限：`User.question_upload_limit`（null=不限制），可在 `/api/users` 与 `/api/project-users` 的下发/编辑接口设置；`POST /api/questions` 按提交者名下题目总数门控，达上限或单次提交超出剩余配额时返回 409 并提示剩余可上传数量。账号列表接口附带 `questionCount`（已上传数）供前端展示消耗。设置上限时（`/api/users` 与 `/api/project-users` 的下发/编辑）会校验项目全部成员上限总和不超过项目题目目标（仅目标 > 0 的项目，未设上限的成员不计入），超出返回 409。项目题目目标门控（上传/质检/重新开始/项目更新时的达标判定、自动暂停与剩余名额）统一按合格题数统计（`project_target_reached` 复用 `count_qualified_questions`，口径同进度展示），不再按题目总数；上传本身不触发自动暂停，合格数达标由 Worker 在分级评测定级落库时同事务触发（`pause_project_if_target_reached`，口径镜像后端：项目级 `qualifiedMinLevel` 覆盖优先、回退全局策略）。
...
- 批量上传用户：`POST /api/users/batch`（管理员）与 `POST /api/project-users/batch`（项目管理员，项目仅限自己所在项目），Excel 固定表头仅“用户名、密码”（2026-08-11 起移除手机号列），密码由文件逐行填写并须满足复杂度（≥8 位且含大小写字母、数字、特殊字符；前端解析时拦截“用户名或密码任一为空/不合规”的行并禁用提交，后端 `batch_create_users_core` 逐行复核、不合规整批 400 带行号），日上限/月上限/月预算作为弹窗内统一输入框对本批次所有账号生效（留空继承全局默认值），题目上传上限同样统一输入（留空不限制），并按本批次实际将创建账号的上限合计经 `validate_question_limit_budget` 校验项目题目目标预算（超出 409），账号固定 `role="user"`；用户名已存在或文件内重复的行跳过并在 `skipped` 中逐条说明，不阻断其余创建。手机号不再进入批量文件，`User.phone`（可空）仍可通过单发（`/api/users`、`/api/project-users`）与编辑接口的 `phone` 字段填写与清空（传 null 清除）。
